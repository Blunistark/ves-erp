...
<body>
    ...
    <div class="unified-container">
        ...
        <div id="teacherDetailsContainer" style="display: none;">
            ...
            <!-- Teacher Basic Information with Edit Mode -->
            <div class="detail-section">
                <div class="detail-header">
                    <i class="fas fa-user"></i>
                    <h3>Teacher Information</h3>
                    <div>
                        <button class="edit-button" onclick="toggleBasicInfoEdit()" id="basicInfoEditBtn">
                ...
                <div class="detail-body">
                    ...
                    <!-- Edit Mode -->
                    <div id="basicInfoEditMode" style="display: none;" class="edit-mode">
                        <form id="basicInfoForm">
                            <div class="info-grid">
                                <div class="form-group">
                                    <label for="editEmployeeNumber" class="form-label">Employee ID</label>
                                    <input type="text" id="editEmployeeNumber" name="employee_number" class="form-input" readonly>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" id="editDateOfBirth" name="date_of_birth" class="form-input">
                                </div>
                                <div class="form-group">
                                    <label for="editQualification" class="form-label">Qualification</label>
                                    <input type="text" id="editQualification" name="qualification" class="form-input" placeholder="e.g., M.A. Mathematics, B.Ed">
                                </div>
                                <div class="form-group">
                                    <label for="editExperience" class="form-label">Experience (Years)</label>
                                    <input type="number" id="editExperience" name="experience_years" class="form-input" min="0" max="50">
                                </div>
                                <div class="form-group">
                                    <label for="editJoinedDate" class="form-label">Joining Date</label>
                                    <input type="date" id="editJoinedDate" name="joined_date" class="form-input">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group" style="grid-column: 1 / -1;">
                                    ...
                                    <textarea id="editAddress" name="address" class="form-textarea" rows="3" placeholder="Enter complete address"></textarea>
                                </div>
                            </div>
                            <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid var(--border-color);">
                                <button type="button" class="save-button" onclick="saveBasicInfo()">
                                    <i class="fas fa-save"></i> Save Changes
                                </button>
                                <button type="button" class="cancel-button" onclick="cancelBasicInfoEdit()">
            ...
            <!-- Password Reset Section (Add this after the Teacher Information section) -->
            <!--<div class="detail-section">-->
            <!--    <div class="detail-header">-->
            ...
            <!--                    <button class="edit-button" onclick="resetTeacherPassword()" style="background: #f59e0b;">-->
            ...
            <div class="detail-section">
                ...
                <div class="detail-body">
                    ...
                    <div id="subjectEditMode" style="display: none;" class="edit-mode">
    ...
    <script>
        ...
        let isBasicInfoEditMode = false;

        function toggleBasicInfoEdit() {
            if (isBasicInfoEditMode) {
                cancelBasicInfoEdit();
            } else {
                isBasicInfoEditMode = true;
                populateBasicInfoForm();
                $('#teacherBasicInfo').hide();
                $('#basicInfoEditMode').show();
                $('#basicInfoEditBtn').html('<i class="fas fa-times"></i> Cancel');
            }
        }

        function populateBasicInfoForm() {
            if (!currentTeacherData || !currentTeacherData.teacher) {
            ...
            $('#editEmployeeNumber').val(teacher.employee_number || '');
            $('#editFullName').val(teacher.full_name || '');
            $('#editEmail').val(teacher.email || '');
            $('#editPhone').val(teacher.phone || '');
            $('#editGender').val(teacher.gender || '');
            $('#editDateOfBirth').val(teacher.date_of_birth || '');
            $('#editQualification').val(teacher.qualification || '');
            $('#editExperience').val(teacher.experience_years || '');
            $('#editJoinedDate').val(teacher.joined_date || '');
            $('#editStatus').val(teacher.status || 'active');
        ...
        function saveBasicInfo() {
            ...
            formData.append('phone', $('#editPhone').val().trim());
            formData.append('gender', $('#editGender').val());
            formData.append('date_of_birth', $('#editDateOfBirth').val());
            formData.append('qualification', $('#editQualification').val().trim());
            formData.append('experience_years', $('#editExperience').val() || 0);
            formData.append('joined_date', $('#editJoinedDate').val());
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        loadTeacherDetails(); // Reload all data
                        cancelBasicInfoEdit();
                    } else {
                        alert('Error: ' + (response.message || 'Failed to update teacher information'));
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Error updating teacher info:', error);
                    let errorMessage = 'Error updating teacher information';
        ...
        function cancelBasicInfoEdit() {
            isBasicInfoEditMode = false;
            $('#basicInfoEditMode').hide();
            $('#teacherBasicInfo').show();
            $('#basicInfoEditBtn').html('<i class="fas fa-edit"></i> Edit');
        }
        ...
        function resetTeacherPassword() {
            ...
            const message = `Are you sure you want to reset the password for ${teacherName}?\n\n` +
                `The password will be reset to their date of birth in DDMMYYYY format.\n` +
                `${teacher.date_of_birth ? 'Current DOB: ' + teacher.date_of_birth : 'No date of birth on file - please update it first.'}\n\n` +
            ...
            if (!teacher.date_of_birth) {
                alert('Cannot reset password: No date of birth found for this teacher. Please update their date of birth first.');
                return;
            }
        ...
        // Update the displayTeacherDetails function to show editable fields properly
        function displayTeacherDetails(data) {
            ...
            // Display basic teacher information with better formatting
            const basicInfoHtml = `
        <div class="info-item">
            <span class="info-label">Employee ID</span>
            <span class="info-value">
                <code style="background: var(--bg-light); padding: 2px 6px; border-radius: 3px;">${teacher.employee_number || 'N/A'}</code>
            </span>
        </div>
        <div class="info-item">
            <span class="info-label">Full Name</span>
            <span class="info-value" style="font-weight: 600;">${teacher.full_name}</span>
        </div>
        <div class="info-item">
            <span class="info-label">Email</span>
            <span class="info-value">
                <a href="mailto:${teacher.email}" style="color: var(--primary-color); text-decoration: none;">${teacher.email}</a>
            </span>
        </div>
        <div class="info-item">
            <span class="info-label">Phone</span>
            <span class="info-value">
                ${teacher.phone ? `<a href="tel:${teacher.phone}" style="color: var(--primary-color); text-decoration: none;">${teacher.phone}</a>` : 'N/A'}
            </span>
        </div>
        <div class="info-item">
            <span class="info-label">Gender</span>
            <span class="info-value">${teacher.gender ? teacher.gender.charAt(0).toUpperCase() + teacher.gender.slice(1) : 'N/A'}</span>
        </div>
        <div class="info-item">
            <span class="info-label">Date of Birth</span>
            <span class="info-value">${teacher.date_of_birth ? new Date(teacher.date_of_birth).toLocaleDateString() : 'N/A'}</span>
        </div>
        <div class="info-item">
        ...
        <div class="info-item">
            ...
            <span class="info-value">${teacher.joined_date ? new Date(teacher.joined_date).toLocaleDateString() : 'N/A'}</span>
        </div>
        <div class="info-item">
            <span class="info-label">Status</span>
            <span class="info-value">
                <span class="status-badge status-${teacher.status}" style="padding: 4px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: 500;">
    ...
    `;
            $('#teacherBasicInfo').html(basicInfoHtml);

            // Continue with existing workload, subjects, class assignments, and timetable display...
            // (Rest of the existing displayTeacherDetails function remains the same)
    ...
    `;
        ...
        function removeClassTeacher(sectionId) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Remove class teacher error:', {
                        xhr,
                        status,
                        error
                    });
