...
const FormValidator = {
    ...
    validateVisitorForm: function(formData) {
        ...
        // Helper to check if a field is empty or only whitespace
        const isEmpty = (value) => value === undefined || value === null || (typeof value === 'string' && value.trim().length === 0);

        if (isEmpty(formData.visitor_name) || formData.visitor_name.trim().length < 2) {
            errors.visitor_name = 'Visitor name must be at least 2 characters long';
        }

        if (isEmpty(formData.phone_number) || !Utils.isValidPhone(formData.phone_number)) {
            errors.phone_number = 'Please enter a valid phone number';
        }

        if (!isEmpty(formData.email) && !Utils.isValidEmail(formData.email)) {
        ...
        if (isEmpty(formData.purpose)) {
            errors.purpose = 'Please select a purpose for the visit';
             ...
             if (isEmpty(formData.other_purpose)) {
                 errors.other_purpose = 'Please specify the purpose of visit';
             }
        }

        if (isEmpty(formData.reporting_person)) {
             errors.reporting_person = 'Please select a reporting person';
        } else if (formData.reporting_person === 'Other') {
             if (isEmpty(formData.other_reporting_person)) {
                 errors.other_reporting_person = 'Please specify the reporting person';
             }
        }

        // Ensure no validation for visit_date remains
        delete errors.visit_date;

        // Log validation results
        if (Object.keys(errors).length > 0) {
            console.error('validateVisitorForm failed. Errors:', errors);
        } else {
            console.log('validateVisitorForm passed.');
        }
    ...
    showFormErrors: function(errors) {
        ...
        Object.keys(errors).forEach(fieldName => {
            ...
            if (field) {
                ...
                // Find the parent .form-field div and append the error message
                const formFieldDiv = field.closest('.form-field');
                ...
                if (formFieldDiv) {
                    ...
                    // Append the error message as the last child of formFieldDiv
                    formFieldDiv.appendChild(errorDiv);
                }
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (visitorForm) {
        visitorForm.addEventListener('submit', function(e) {
            ...
            if (Object.keys(errors).length > 0) {
                ...
                // Log the final message triggering error
                console.log('Triggering error message: Please correct the errors below.');
                return;
            }
             // Log successful validation if reached
    ...
    document.querySelectorAll('.form-input').forEach(input => {
        input.addEventListener('input', function() {
            // Clear existing errors related to this input
            const formFieldDiv = this.closest('.form-field');
            if (formFieldDiv) {
                const existingError = formFieldDiv.querySelector('.form-error');
                if (existingError) {
            ...
            // Re-validate the form on input change is complex and was causing issues.
            // Rely solely on the form submit listener for comprehensive validation.
            // The generic error message at the top is cleared on any input change.
            Utils.clearMessages();
        });
    });
});
