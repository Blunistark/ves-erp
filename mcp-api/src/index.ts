...
async function makeHttpRequest(
  ...
  try {
    ...
    const authenticated = !(
      ...
      response.status === 401 ||
      (response.status === 302 && (
        responseHeaders['location']?.includes('login') ||
...
async function attemptAutoLogin(baseUrl: string, targetUrl?: string): Promise<{ success: boolean; userType?: string; attempts: any[] }> {
  // Determine which portal to try based on the target URL
  const getPortalFromUrl = (url: string): string => {
        ...
        if (loginPageResponse.status === 200) {
          ...
          if (endpoint.portal === 'admin') {
            ...
            Object.entries(credentials).forEach(([key, value]) => {
              ...
              if (key === 'email') {
                fieldName = possibleFields.find(f => 
...
function formatResponse(response: any, authAttempts?: any[]): string {
  ...
  if (authAttempts && authAttempts.length > 0) {
    authDetails = `\n**Authentication Attempts:**\n${authAttempts.map(attempt => 
      `  ${attempt.portal}: ${attempt.status || 'failed'} ${attempt.error ? `(${attempt.error})` : ''}`
    ).join('\n')}\n`;
  }
...
server.tool(
  "api_get",
  "Make a GET request to an API endpoint with automatic authentication",
  {
...
server.tool(
  "api_post",
  "Make a POST request to an API endpoint with automatic authentication",
  {
...
server.tool(
  "api_check",
  "Check an API endpoint with customizable HTTP method and automatic authentication",
  {
    url: z.string().url().describe("The API URL to check"),
